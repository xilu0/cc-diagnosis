# Claude Code 调试指南

[English](DEBUG.md) | 简体中文

全面的指南，用于诊断和排查 Claude Code 的性能问题、无响应和卡顿进程。

## 目录

- [快速开始](#快速开始)
- [调试标志参考](#调试标志参考)
- [日志文件和位置](#日志文件和位置)
- [实时监控](#实时监控)
- [故障排查决策树](#故障排查决策树)
- [常见问题和解决方案](#常见问题和解决方案)
- [性能优化](#性能优化)
- [社区工具](#社区工具)

---

## 快速开始

当 Claude Code 看起来卡住或缓慢时,立即运行：

```bash
# 启用所有调试输出
claude --verbose --debug

# 对于 MCP 相关问题
claude --verbose --mcp-debug

# 将调试输出保存到文件
claude --verbose --debug > debug.log 2>&1
```

**快速诊断命令：**

```bash
claude doctor          # 系统健康检查
claude /status         # 检查当前状态
claude mcp list        # MCP 服务器状态
claude /cost           # 查看会话成本
```

---

## 调试标志参考

### `--verbose`

**用途**：显示内部操作的详细诊断输出

**显示内容：**
- 每个操作的时间戳
- 工具执行和参数
- 文件操作（读取、写入、编辑）
- API 通信详情
- 操作状态和结果

**用法：**
```bash
claude --verbose
claude --verbose [command]
claude --verbose -p "你的提示词"
```

**最适合：**
- 了解 Claude Code 正在做什么
- 识别瓶颈
- 调试慢响应
- 跟踪工具调用

**示例输出：**
```
[2025-01-14 10:30:45] API 请求: /v1/messages
[2025-01-14 10:30:46] 工具调用: read_file(/path/to/file.js)
[2025-01-14 10:30:47] 文件读取: 1234 字节
[2025-01-14 10:30:48] API 响应: 200 OK (tokens: 1500)
```

### `--debug`

**用途**：启用带详细调试输出的调试模式

**显示内容：**
- 内部状态信息
- 错误堆栈跟踪
- 配置值
- 环境变量

**用法：**
```bash
claude --debug
claude --debug [command]
```

**最适合：**
- 排查错误
- 理解配置问题
- 开发者级别的调试

### `--mcp-debug`

**用途**：专门用于 Model Context Protocol 连接的调试

**显示内容：**
- MCP 服务器初始化
- 连接尝试和状态
- MCP 服务器的工具可用性
- MCP 相关的错误和警告

**用法：**
```bash
claude --mcp-debug
claude --verbose --mcp-debug  # 推荐组合
```

**最适合：**
- MCP 服务器连接问题
- 缺失或不可用的 MCP 工具
- MCP 配置问题

**检测到的常见问题：**
- MCP 服务器启动失败
- 从预期的 MCP 服务器找不到工具
- MCP 服务器连接超时

### 组合使用标志

为获得全面的诊断，组合使用多个标志：

```bash
# 最大详细程度
claude --verbose --debug --mcp-debug

# 大多数调试场景的推荐组合
claude --verbose --mcp-debug
```

---

## 日志文件和位置

### 主要日志位置

Claude Code 以 JSONL（换行分隔的 JSON）格式存储完整的会话日志：

**路径：**
```bash
~/.claude/projects/<encoded-directory>/*.jsonl
```

**配置文件：**
```bash
~/.claude/settings.json              # 全局设置
.claude/settings.json                # 项目特定设置
```

### 查找你的日志

```bash
# 方法 1：使用 doctor 命令
claude doctor
# 输出将显示："日志目录: /Users/username/.claude/projects/..."

# 方法 2：列出最近的日志
ls -lt ~/.claude/projects/*/
```

### 查看日志

**原始 JSONL 格式：**
```bash
# 查看最新的日志文件
tail -f ~/.claude/projects/*/$(ls -t ~/.claude/projects/*/ | head -1)

# 使用 jq 美化输出
tail -f ~/.claude/projects/*/*.jsonl | jq '.'
```

**过滤特定事件：**
```bash
# 仅显示 API 请求
cat ~/.claude/projects/*/*.jsonl | jq 'select(.type == "api_request")'

# 仅显示错误
cat ~/.claude/projects/*/*.jsonl | jq 'select(.level == "error")'

# 显示工具调用
cat ~/.claude/projects/*/*.jsonl | jq 'select(.type == "tool_call")'
```

### 日志轮换和清理

日志可能会随时间积累。管理空间的方法：

```bash
# 检查日志目录大小
du -sh ~/.claude/

# 删除 30 天前的日志
find ~/.claude/projects -name "*.jsonl" -mtime +30 -delete

# 每个项目仅保留最近 10 个会话
# （手动清理 - 删除前请先检查）
ls -t ~/.claude/projects/*/session*.jsonl | tail -n +11 | xargs rm
```

---

## 实时监控

### 内置监控命令

在活跃的 Claude Code 会话期间使用这些命令：

```bash
/status              # 当前会话状态
/cost                # Token 使用量和成本
/permissions         # 当前权限设置
/clear               # 清除上下文窗口
/compact             # 压缩对话历史
```

### 基于终端的监控

**实时查看日志文件：**
```bash
# 在单独的终端窗口中
tail -f ~/.claude/projects/*/*.jsonl | jq -r '.timestamp + " " + .type + " " + (.message // "")'
```

**监控 API 调用：**
```bash
# 过滤 API 通信
tail -f ~/.claude/projects/*/*.jsonl | jq 'select(.type | test("api"))'
```

**跟踪工具执行：**
```bash
# 监控工具调用
tail -f ~/.claude/projects/*/*.jsonl | jq 'select(.type == "tool_call") | {time: .timestamp, tool: .tool_name, duration: .duration}'
```

### 社区监控工具

#### 1. claude-code-logger（基于代理）

拦截并显示 Claude Code 和 API 之间的流量。

**安装：**
```bash
git clone https://github.com/username/claude-code-logger
cd claude-code-logger
npm install
npm run dev -- start --chat-mode
```

**使用：**
```bash
# 终端 1：启动代理
npm run dev -- start --chat-mode

# 终端 2：使用代理运行 Claude
ANTHROPIC_BASE_URL=http://localhost:8000/ claude
```

**功能：**
- 实时请求/响应流式传输
- 带彩色输出的聊天模式
- 请求/响应格式化
- 延迟跟踪

#### 2. claude-code-log（TUI 查看器）

用于浏览会话的交互式终端 UI。

**安装：**
```bash
pipx install claude-conversation-extractor
# 或
pip install claude-conversation-extractor
```

**使用：**
```bash
# 交互式 TUI
claude-code-log --tui

# 启动会话浏览器
claude-start
```

**功能：**
- 带 ID、摘要、时间戳的会话列表
- 消息计数和 Token 使用量
- 搜索和过滤功能
- 导出对话

#### 3. 实时仪表板（自定义脚本）

创建一个简单的监控脚本：

```bash
#!/bin/bash
# 保存为 monitor-claude.sh

LOG_DIR="$HOME/.claude/projects"
LATEST_LOG=$(find "$LOG_DIR" -name "*.jsonl" -type f -printf '%T@ %p\n' | sort -n | tail -1 | cut -f2- -d" ")

echo "监控中: $LATEST_LOG"
echo "按 Ctrl+C 停止"
echo ""

tail -f "$LATEST_LOG" | while read line; do
    TYPE=$(echo "$line" | jq -r '.type // "unknown"')
    TIMESTAMP=$(echo "$line" | jq -r '.timestamp // "no-time"')

    case "$TYPE" in
        "api_request")
            echo "[$TIMESTAMP] 🌐 API 请求"
            ;;
        "api_response")
            TOKENS=$(echo "$line" | jq -r '.usage.total_tokens // "?"')
            echo "[$TIMESTAMP] ✅ API 响应 (tokens: $TOKENS)"
            ;;
        "tool_call")
            TOOL=$(echo "$line" | jq -r '.tool_name // "unknown"')
            echo "[$TIMESTAMP] 🔧 工具: $TOOL"
            ;;
        "error")
            MSG=$(echo "$line" | jq -r '.message // "unknown error"')
            echo "[$TIMESTAMP] ❌ 错误: $MSG"
            ;;
    esac
done
```

添加执行权限并运行：
```bash
chmod +x monitor-claude.sh
./monitor-claude.sh
```

---

## 故障排查决策树

### 当 Claude Code 看起来卡住时

```
┌─────────────────────────────────────┐
│ Claude Code 看起来无响应           │
└──────────────┬──────────────────────┘
               │
               ▼
┌─────────────────────────────────────┐
│ 启用: claude --verbose --debug     │
└──────────────┬──────────────────────┘
               │
               ▼
┌─────────────────────────────────────┐
│ 观察输出 30-60 秒                  │
└──────────────┬──────────────────────┘
               │
               ▼
        ┌──────┴──────┐
        │             │
        ▼             ▼
  ┌─────────┐   ┌─────────┐
  │ 有输出？│   │ 无输出？│
  └────┬────┘   └────┬────┘
       │             │
       │             └─────────────┐
       │                           │
       ▼                           ▼
┌──────────────────┐    ┌──────────────────────┐
│ 什么类型的       │    │ 检查日志文件:        │
│ 输出？           │    │ tail -f ~/.claude/   │
└────┬─────────────┘    │      projects/*/*.jsonl│
     │                  └──────────┬─────────────┘
     │                             │
     ├─ API 请求/响应              ├─ 文件在增长？
     │  → 正常处理中               │  是 → 后台处理中
     │  → 继续等待                 │  否 → 进程挂起
     │                             │       → Ctrl+C 重试
     ├─ 工具调用
     │  → 检查是否循环
     │  → 如果卡住，Ctrl+C
     │  → 如果是 MCP 工具，检查：
     │     claude mcp list
     │
     ├─ 错误消息
     │  → 处理特定错误
     │  → 参见下面的常见问题
     │
     └─ 超时警告
        → 网络问题
        → 检查连接性
        → 运行网络诊断
```

### 快速决策点

| 症状 | 检查这个 | 行动 |
|------|---------|------|
| 2 分钟以上无输出 | 日志文件还在增长？ | 是：等待；否：Ctrl+C 重试 |
| 重复的工具调用 | 同一工具被调用 10 次以上？ | 可能是循环，按 Ctrl+C |
| API 超时 | 网络连接性 | 运行 `./diagnose.sh` 进行网络检查 |
| MCP 错误 | `claude mcp list` | 重启受影响的 MCP 服务器 |
| 高内存使用 | 上下文窗口大小 | 运行 `/clear` 或 `/compact` |
| 响应缓慢 | Token 计数 | 检查 `/cost`，考虑 `/clear` |

---

## 常见问题和解决方案

### 问题 1：Claude Code 看起来冻结

**症状：**
- 没有可见的进展
- 没有终端输出
- 光标只是闪烁

**诊断：**
```bash
# 终端 1：使用 verbose 运行
claude --verbose

# 终端 2：监控日志
tail -f ~/.claude/projects/*/*.jsonl | jq -r '.timestamp + " " + .type'
```

**解决方案：**

**如果日志显示活动：**
- Claude 正在工作，请耐心等待
- 大文件操作需要时间
- 复杂的推理需要处理

**如果日志是静态的：**
```bash
# 按 Ctrl+C 中断
# 然后清除上下文后重试
claude
/clear
# 重试你的请求
```

### 问题 2：响应时间慢

**症状：**
- 每个响应需要 30 秒以上
- 思考似乎很慢
- 工具调用被延迟

**诊断：**
```bash
# 检查上下文大小
/cost  # 显示使用的 tokens

# 检查上下文膨胀
/status
```

**解决方案：**

1. **定期清除上下文：**
```bash
/clear     # 完全重置上下文
/compact   # 压缩历史记录同时保持上下文
```

2. **优化提示词：**
```bash
# 不好：模糊且需要探索
"修复所有 bug"

# 好：具体且有针对性
"修复 src/utils/parser.js 第 45 行的 TypeError"
```

3. **使用 CLAUDE.md 提供上下文：**

在项目根目录创建 `.claude/CLAUDE.md`：
```markdown
# 项目：MyApp

## 结构
- src/ - 源代码
- tests/ - 测试文件
- docs/ - 文档

## 常用命令
- npm test - 运行测试
- npm build - 构建项目

## 代码风格
- 使用 TypeScript
- 偏好函数式风格
- 最大行长度：100

## 已知问题
- API 速率限制：100 请求/分钟
- 数据库：PostgreSQL 14+
```

### 问题 3：MCP 服务器不工作

**症状：**
- 工具不可用
- MCP 连接错误
- "服务器无响应"

**诊断：**
```bash
# 检查 MCP 服务器状态
claude mcp list
claude --mcp-debug
```

**预期输出：**
```
✓ github - 已连接（15 个工具可用）
✓ filesystem - 已连接（8 个工具可用）
✗ custom-server - 连接失败
```

**解决方案：**

1. **重启单个 MCP 服务器：**
```bash
# 检查 MCP 配置
cat ~/.claude/mcp-servers.json

# 手动测试服务器
node /path/to/mcp-server/index.js
```

2. **检查 MCP 服务器日志：**
```bash
# MCP 服务器通常记录到 stderr
# 检查 Claude Code 日志中的 MCP 错误
cat ~/.claude/projects/*/*.jsonl | jq 'select(.type == "mcp_error")'
```

3. **验证 MCP 配置：**
```json
// ~/.claude/mcp-servers.json
{
  "mcpServers": {
    "filesystem": {
      "command": "node",
      "args": ["/path/to/filesystem-server/index.js"]
    }
  }
}
```

### 问题 4：内存不足错误

**症状：**
- 进程崩溃
- "JavaScript heap out of memory"
- 系统变慢

**诊断：**
```bash
# 检查 Node.js 内存使用
ps aux | grep claude

# 在执行期间监控内存
while true; do
  ps aux | grep claude | grep -v grep | awk '{print $6/1024 " MB"}'
  sleep 2
done
```

**解决方案：**

1. **增加 Node.js 内存：**
```bash
# 设置内存限制（8GB 示例）
export NODE_OPTIONS="--max-old-space-size=8192"
claude
```

2. **频繁清除上下文：**
```bash
# 每完成一个主要任务后使用 /clear
/clear
```

3. **避免加载大文件：**
```bash
# 不好：加载整个 50MB 日志文件
# 好：让 Claude 读取特定行或先使用 grep
```

### 问题 5：Token 限制超出

**症状：**
- "上下文窗口超出"
- "达到最大 tokens"
- 响应被截断

**诊断：**
```bash
/cost   # 检查当前 token 使用量
```

**解决方案：**

1. **使用 /compact 而不是 /clear：**
```bash
/compact  # 保留重要上下文，删除冗余内容
```

2. **分解任务：**
```bash
# 不要这样："重构整个代码库"
# 而是这样："重构 src/auth.js"，然后 "重构 src/api.js"，等等
```

3. **使用选择性文件读取：**
```bash
# 引导 Claude 只读取相关文件
"只读取 src/ 中与身份验证相关的文件"
```

---

## 性能优化

### 最佳实践

#### 1. 上下文管理

```bash
# 工作会话开始
claude

# 完成主要任务后
/clear

# 切换到不同任务前
/compact
```

#### 2. 有效的提示

**不好的提示（慢，资源密集）：**
- "修复所有问题"
- "让这个更好"
- "审查所有内容"

**好的提示（快，有针对性）：**
- "修复 auth.js:42 的空指针错误"
- "为登录函数添加错误处理"
- "审查密码重置流程的安全性"

#### 3. CLAUDE.md 配置

为 Claude 创建有用的上下文：

```markdown
# 项目配置

## 快速事实
- 语言：TypeScript
- 框架：React + Express
- 数据库：PostgreSQL
- 测试：Jest

## 文件结构
```
src/
├── api/       # REST API 端点
├── components/  # React 组件
├── utils/     # 辅助函数
└── types/     # TypeScript 类型
```

## 常用命令
- `npm test` - 运行所有测试
- `npm run dev` - 启动开发服务器
- `npm run build` - 生产构建

## 编码标准
- 最大函数长度：50 行
- 优先使用组合而非继承
- 所有导出必须有 JSDoc 注释

## 调试说明
- API 服务器运行在 3000 端口
- 数据库连接：localhost:5432
- Redis 缓存：localhost:6379
```

#### 4. 限制工具执行

```bash
# 使用 --max-turns 防止失控执行
claude --max-turns 5 -p "你的复杂任务"
```

#### 5. 监控资源使用

```bash
# 创建监控别名
alias claude-monitor='watch -n 1 "ps aux | grep claude | grep -v grep"'

# 在开始密集工作前运行
claude-monitor
```

### API 响应延迟测试

测试 API 响应时间有助于识别速度慢是由网络问题、API 服务器性能还是本地处理造成的。

#### 测量命令执行时间

**macOS/Linux:**
```bash
# 测试基本的 API 响应时间
time claude -p "hi"

# 示例输出:
# real    0m2.347s    # 总耗时（挂钟时间）
# user    0m0.234s    # 用户模式下的 CPU 时间
# sys     0m0.056s    # 系统模式下的 CPU 时间
```

**Windows (PowerShell):**
```powershell
# 测试基本的 API 响应时间
Measure-Command { claude -p "hi" }

# 示例输出:
# TotalSeconds      : 2.3471234
# TotalMilliseconds : 2347.1234
```

#### 解读结果

| 响应时间 | 状态 | 可能原因 |
|---------|------|---------|
| < 10 秒 | 优秀 | 正常运行 |
| 12-15 秒 | 良好 | 典型的 API 延迟 |
| 15-20 秒 | 较慢 | 网络拥塞或 API 负载 |
| 20-30 秒 | 非常慢 | 连接问题，检查网络 |
| > 30 秒 | 超时风险 | 运行 `./diagnose.sh` 进行网络诊断 |

#### 基线测试

为你的环境建立基线:

```bash
# macOS/Linux: 运行多次测试
for i in {1..5}; do
  echo "测试 $i:"
  time claude -p "hi" 2>&1 | grep real
done

# Windows (PowerShell): 运行多次测试
1..5 | ForEach-Object {
  Write-Host "测试 $_:"
  (Measure-Command { claude -p "hi" }).TotalSeconds
}
```

#### 排查高延迟问题

**如果延迟持续很高 (>10秒):**

1. **检查网络连接:**
```bash
# 运行诊断脚本
./diagnose.sh  # macOS/Linux
.\diagnose.ps1  # Windows
```

2. **测试直接 API 访问:**
```bash
# macOS/Linux
curl -w "\n时间: %{time_total}s\n" \
  -H "x-api-key: $ANTHROPIC_AUTH_TOKEN" \
  -H "anthropic-version: 2023-06-01" \
  https://claude-code.club/api/v1/models

# Windows (PowerShell)
Measure-Command {
  Invoke-WebRequest -Uri "https://claude-code.club/api/v1/models" `
    -Headers @{
      "x-api-key" = $env:ANTHROPIC_AUTH_TOKEN
      "anthropic-version" = "2023-06-01"
    }
}
```

3. **检查代理/VPN 干扰:**
```bash
# 临时禁用代理
unset HTTP_PROXY HTTPS_PROXY http_proxy https_proxy

# 再次测试
time claude -p "hi"
```

**如果延迟变化很大:**
- 可能表明网络拥塞
- 检查时间段模式
- 考虑到 API 服务器的地理距离
- 检查防火墙/杀毒软件设置

### 配置优化

**全局设置（`~/.claude/settings.json`）：**

```json
{
  "verbose": false,
  "auto_clear_context": true,
  "max_context_tokens": 100000,
  "tool_timeout": 30000
}
```

**项目设置（`.claude/settings.json`）：**

```json
{
  "ignored_paths": [
    "node_modules/",
    "dist/",
    "build/",
    "*.log",
    ".git/"
  ],
  "max_file_size": 1048576
}
```

---

## 社区工具

### 1. claude-code-logger

**用途**：HTTP 代理监控 API 流量

**安装：**
```bash
git clone https://github.com/anthropics/claude-code-logger
cd claude-code-logger
npm install
```

**使用：**
```bash
npm run dev -- start --chat-mode
ANTHROPIC_BASE_URL=http://localhost:8000/ claude
```

**功能：**
- 实时请求/响应显示
- Token 计数
- 延迟测量
- 聊天式格式化

### 2. claude-conversation-extractor

**用途**：提取和浏览 Claude Code 会话

**安装：**
```bash
pipx install claude-conversation-extractor
```

**使用：**
```bash
# 交互式 TUI
claude-code-log --tui

# 导出特定对话
claude-code-log export --id <session-id> --format markdown
```

**功能：**
- 会话搜索
- Token 统计
- 导出为多种格式
- 对话回放

### 3. 自定义监控脚本

**实时 token 计数器：**
```bash
#!/bin/bash
# live-cost.sh

while true; do
  LOG=$(find ~/.claude/projects -name "*.jsonl" -type f -printf '%T@ %p\n' | sort -n | tail -1 | cut -f2- -d" ")
  TOKENS=$(tail -1000 "$LOG" | jq -s '[.[].usage.total_tokens // 0] | add')
  echo "最近 1000 个事件的总 tokens: $TOKENS"
  sleep 5
done
```

**错误跟踪器：**
```bash
#!/bin/bash
# track-errors.sh

LOG_DIR="$HOME/.claude/projects"

echo "监控错误中..."
tail -f "$LOG_DIR"/*/*.jsonl | jq -r 'select(.level == "error") | .timestamp + " 错误: " + .message'
```

---

## 附录：调试检查清单

在故障排查时，按照这个检查清单逐项检查：

- [ ] 已启用 `--verbose` 标志
- [ ] 已检查日志文件正在被写入
- [ ] 已验证网络连接（运行 `./diagnose.sh`）
- [ ] 已确认 MCP 服务器正在运行（`claude mcp list`）
- [ ] 已检查上下文大小（`/cost`）
- [ ] 如需要已清除上下文（`/clear`）
- [ ] 已查看日志中的最近错误
- [ ] 已验证磁盘空间可用
- [ ] 已检查系统内存使用
- [ ] 已用最小提示测试
- [ ] 已查看 CLAUDE.md 配置（如果存在）
- [ ] 已确认 API 密钥有效

---

## 获取帮助

如果按照本指南操作后问题仍然存在：

1. **收集调试信息：**
```bash
# 生成综合调试报告
claude doctor > claude-debug-report.txt
claude --verbose --debug [your-command] >> claude-debug-report.txt 2>&1
```

2. **查看社区资源：**
- GitHub Issues：https://github.com/anthropics/claude-code/issues
- Discord：Claude Code 社区频道
- 文档：https://docs.anthropic.com

3. **提交 bug 报告时附上：**
- 调试报告输出
- 相关日志摘录
- 复现步骤
- 系统信息（操作系统、Node 版本、Claude Code 版本）

---

**最后更新**：2025-01-14

关于网络连接问题，请参阅 [README_CN.md](README_CN.md) 并使用诊断脚本（`diagnose.sh` 或 `diagnose.ps1`）。
